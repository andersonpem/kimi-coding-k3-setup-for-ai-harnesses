...
The scripts configure the Kimi Code API, expose the full Kimi Code model lineup (`k3`, `k3-256k`, `kimi-for-coding`, `kimi-for-coding-highspeed`) with `k3-256k` as the default, and prompt for the API key without echoing it to the terminal.
...
|-------------|-----------------------|---------------------------------|----------------------|-------------------------------------------------------------|
| Claude Code | `scripts/claude.sh`   | `scripts/claude-uninstall.sh`   | Anthropic-compatible | Default `k3-256k`, Opus/Sonnet `k3[1m]`, Haiku `kimi-for-coding-highspeed`, Fable `kimi-for-coding` |
| OpenCode    | `scripts/opencode.sh` | `scripts/opencode-uninstall.sh` | OpenAI-compatible    | Default `kimi-code/k3-256k`, plus `k3`, `kimi-for-coding`, `kimi-for-coding-highspeed` |
...
- Creates `~/.config/kimi-claude/env.sh`, sets its permissions to `600`, and stores the API endpoint, key, per-tier model aliases, and context settings there. The default (main) model is `k3-256k`; the Opus and Sonnet tiers use `k3[1m]`, Haiku uses `kimi-for-coding-highspeed`, and Fable uses `kimi-for-coding`.
...
- Writes `~/.config/opencode/opencode.jsonc` with Kimi Code as the provider and `k3-256k` as the default model, alongside `k3`, `kimi-for-coding`, and `kimi-for-coding-highspeed`.
